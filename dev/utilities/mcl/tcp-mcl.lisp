...
(defmethod start-server (name listen-port connection-handler &rest handler-args)
  (process-run-function 
   ...
   (lambda ()                     
     (loop do
           (with-open-socket (stream nil listen-port)
             (setf (ccl::stream-read-timeout stream) nil)
             (process-wait "Listen" (lambda (s) (listen s)) stream)
             (let ((socket-stream (open-socket-stream 
                                   nil listen-port :element-type 'base-char)))
               (make-thread 
                name 
                (lambda ()
                  (apply connection-handler socket-stream handler-args)))))))))
